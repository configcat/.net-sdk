...
public class ConfigCatClientTests
{
    ...
    public async Task WaitForReadyAsync_ShouldBeCanceledIfClientGetsDisposedDuringInitialAutoPollFetch()
    {
        ...
        var configFetcher = new DefaultConfigFetcher("x", new Uri("http://example.com"), "1.0", loggerWrapper, new HttpClientConfigFetcher(fakeHandler), true, false, TimeSpan.FromMilliseconds(delayMs * 2));
