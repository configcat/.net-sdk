...
public class ConfigCatClientOptions : IProvidesHooks
{
    ...
    internal static IConfigCatConfigFetcher CreateDefaultConfigFetcher(HttpClientHandler? httpClientHandler, IWebProxy? proxy, LoggerWrapper logger) =>
        httpClientHandler is null ? new HttpClientConfigFetcher(proxy, logger) : new HttpClientConfigFetcher(httpClientHandler);
    ...
    [Obsolete($"This option is deprecated, thus it will be removed from the public API in a future major version. Please set the {nameof(Proxy)} option instead, or use the {nameof(ConfigFetcher)} (e.g. set it to an instance of {nameof(HttpClientConfigFetcher)}) if you need more control over HTTP communication.")]
    /// <summary>
